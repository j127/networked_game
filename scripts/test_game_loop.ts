...
async function runSimulation() {
  ...
  if (game?.current_phase !== "INCOME")
    throw new Error("Phase should be INCOME");
  ...
  game = getGame(gameId);
  if (game?.current_phase !== "EVENTS")
    throw new Error("Phase should be EVENTS");
  ...
  if (game?.current_phase !== "ACQUIRE")
    throw new Error("Phase should be ACQUIRE");
  ...
  if (p1?.gold !== 5)
    console.warn(`⚠️ Unexpected Gold: ${p1?.gold}. Expected 5.`);
  ...
  game = getGame(gameId);
  if (game?.current_phase !== "INCOME")
    throw new Error("Phase should be INCOME (Turn 2)");
